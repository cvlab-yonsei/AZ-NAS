...
def compute_nas_score(model, gpu, trainloader, resolution, batch_size, init_method = 'kaiming_norm_fanin', fp16=False):
    ...
    fwrd_time = start.elapsed_time(end) ########################################### end ###########################################
    ...
    torch.cuda.synchronize()
    exp_time = start.elapsed_time(end)
    #####################################################################
    ...
    for i in reversed(range(1, len(layer_features))):
        ...
        torch.cuda.synchronize()
        bkwd_time.append(start.elapsed_time(end)) ########################################### end ###########################################
        ...
        torch.cuda.synchronize()
        trn_time.append(start.elapsed_time(end)) ########################################### end ###########################################
    trainability = np.mean(scores)
    #################################################

    info['fwrd_time'] = fwrd_time
    info['exp_time'] = exp_time
    info['bkwd_time'] = np.sum(bkwd_time)
    info['trn_time'] = np.sum(trn_time)
