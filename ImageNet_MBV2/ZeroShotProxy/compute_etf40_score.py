...
def compute_nas_score(model, gpu, trainloader, resolution, batch_size, fp16=False):
    ...
    #                 scores.append(p.grad.abs().mean().item() / p.abs().mean().item())
    # grad_variance = -np.std(scores)
    ...
    info['mean_update'] = float(mean_update) if not np.isnan(mean_update) else -np.inf
