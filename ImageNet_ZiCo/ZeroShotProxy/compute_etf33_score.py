...
def compute_nas_score(model, gpu, trainloader, resolution, batch_size, fp16=False):
    ...
    #             s = f_out.norm(p=2, dim=(1)).mean() / (f_in.norm(p=2, dim=(1)).mean()+1e-6) * np.sqrt(ci/co)
    ...
    g = torch.randn_like(last_feat)
    g = g / (g.norm(p=2, dim=(1),keepdim=True)+1e-6)
    last_feat.backward(g)
    norm2_sum = 0
    with torch.no_grad():
    ...
    ################ bkwd norm score ##############
    """
    grad stability score across residual block features / consider channel diff
    """
    scores = []
    for i in reversed(range(1, len(layer_features))):
        ...
        if g_out.size()==g_in.size() and torch.all(g_in == g_out):
            ...
            s = g_out.norm(p=2, dim=(1)).mean() / (g_in.norm(p=2, dim=(1)).mean()+1e-6) * np.sqrt(ci/co)
            scores.append(-s.item() - 1/(s.item()+1e-6) + 2)
