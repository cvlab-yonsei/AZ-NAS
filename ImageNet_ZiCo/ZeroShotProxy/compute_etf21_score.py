...
def compute_nas_score(model, gpu, trainloader, resolution, batch_size, fp16=False):
    ...
    layer_features.insert(0, input_)
    
    ################ fwrd pca score ################
    """
    pca score across residual block features / normalize each score by upper bound / exclude the last feat
    """
    scores = []
    for i in range(len(layer_features[1:-1])):
        ...
        u, s, v = torch.svd(sigma.cpu(), compute_uv=False)
        prob_s = s / s.sum()
    ...
    ################ fwrd norm score ################
    """
    residual block features / consider difference in channel-width / include input img & last feat
    """
    scores = []
    for i in range(1, len(layer_features)):
        ...
        if (f_out.size() == f_in.size()) and (torch.all(f_in == f_out)):
            ...
            #     stride = int(f_in.size(2)/f_out.size(2))
    ...
    ################ spec norm score ##############
    """
    spec norm score across residual block features / consider difference in channel-width / match res by stacking / include input img & last feat
    """
    scores = []
    for i in reversed(range(1, len(layer_features))):
        ...
        if g_out.size()==g_in.size() and torch.all(g_in == g_out):
            ...
            ss = s.max().item() * np.sqrt(min(ci,co)/max(ci,co))
