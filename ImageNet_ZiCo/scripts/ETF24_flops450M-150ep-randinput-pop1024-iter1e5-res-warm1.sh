...
save_dir=./save_dir/ETF24_flops450M-resblockfeat-searchbs128-randinput-pop1024-iter1e5-res
mkdir -p ${save_dir}
...
horovodrun -np 8 python train_image_classification.py --dataset imagenet --num_classes 1000 \
  ...
  --input_image_size ${resolution} --epochs ${epochs} --warmup 1 \
  --optimizer sgd --bn_momentum 0.01 --wd 4e-5 --nesterov --weight_init custom_xavier \
  --label_smoothing \
  ...
  --batch_size_per_gpu 64 --save_dir ${save_dir}/plain_training_epochs${epochs}_labelsmoothing_xavier_warm1 \
  --world-size 8 \
  --dist_mode horovod\
