...
def train_one_epoch(writer, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if not math.isfinite(loss_value):
            ...
            # sys.exit(1)
            flag_tensor += 1
            print("Warning: Loss is {}, skip this iteration and setting amp=False".format(loss_value))
            logging.info("Warning: Loss is {}, skip this iteration and setting amp=False".format(loss_value))
